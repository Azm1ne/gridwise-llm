...
> [!IMPORTANT]
> For `operator_notes`, supported directive types, structured adjustments, guardrails, request/response fields, battery rules, and exact challenge behavior, read the separate **Problem Statement**.
...
| Runtime training | Do not require long training or fine-tuning jobs during the judging window. |
| Docker fallback | Submit a tested image with an exact pullable tag or digest. The image must reach `/health` when run with the documented command. |
| Response time | `POST /optimize-energy` must complete within 30 seconds per request. |
| No hard-coding | Do not hard-code public sample phrasing; hidden notes may paraphrase the same directive. |
...
The language model must be on the operator-note interpretation path that produces `directive_interpretation`. Using an LLM only for `plan_summary`, documentation, or general text does not satisfy the requirement.

| Acceptable usage | Not acceptable |
|---|---|
| LLM reads operator notes and outputs `directive_interpretation` used by the optimizer. | AI used only for `plan_summary` or documentation — does not satisfy the LLM requirement. The model must interpret `operator_notes` into `directive_interpretation` used by the optimizer. |
...
|---|---|
| **LLM Directive Interpretation** · 25 pts · Interpretation Accuracy | 25 = 5 relevance/`no_op` + 5 `directive_type` + 5 affected hours + 5 numeric value + 5 `structured_adjustment` shape. Each sub-criterion is awarded per hidden case; partial credit is possible. `no_op` must use `applies = false` and `structured_adjustment = null`. All other directives must use `applies = true`. |
| **Directive Application & Constraint Correctness** · 25 pts · Schedule Validity | 25 = correctness of applying the organizer ground-truth directive to the final `hourly_plan`. The judge applies the organizer ground-truth directive and checks whether `hourly_plan` satisfies it plus all GridWise constraints. |
| **Optimization Quality** · 10 pts · Cost Quality | `quality_ratio = min(1, organizer_optimal_cost / recalculated_team_cost)`. Score = 10 × average `quality_ratio` across all optimization hidden cases. Only valid schedules (correct directives + GridWise rules) are scored for cost quality. |
| **API Contract & Schema** · 10 pts · Structural Conformance | 10 = exact `directive_interpretation` structure per note + exact `hourly_plan` with 24 entries + recalculated summary fields (`total_grid_kwh`, `total_cost_bdt`, `peak_grid_kwh`) within tolerance + health endpoint contract. |
| **Performance & Reliability** · 10 pts · Operational Stability | 10 = 3 p95 latency + 4 failure rate + 3 malformed-input handling. p95 ≤ 5s: 3/3; >5s to 15s: 2/3; >15s to 30s: 1/3; >30s: 0/3. Repeated failures and timeouts reduce scores. |
| **Deployment & Docker Fallback** · 10 pts · Reproducibility Criteria | 10 = 4 pullable image with exact tag/digest + 3 documented `docker run` command reaches `/health` + 2 no baked-in secrets + 1 exposed port and `0.0.0.0` binding. Evaluated from submitted artifacts. |
...
> [!NOTE]
> **OPTIMIZATION SCORE:** `min(1, organizer_optimal_cost / recalculated_team_cost)`. Optimization Quality = 10 × the average `quality_ratio` across all optimization hidden cases. If `organizer_optimal_cost` and `recalculated_team_cost` are both within numeric tolerance of 0, `quality_ratio = 1`. If `organizer_optimal_cost` is within tolerance of 0 but team cost is above tolerance, `quality_ratio = 0`.
...
|---|---|---|
| Required LLM absent from operator-note interpretation path, or AI used only for `plan_summary`/documentation | Fails the mandatory challenge requirement; not eligible for the final preliminary shortlist | Automated and artifact verification may inspect the repository/architecture to confirm that a language-capable generative model is on the `directive_interpretation` path. |
| Wrong directive type or incorrect hours/values (e.g., treating `no_charge_window` as `no_discharge_window`) | 0 points for that interpretation sub-criterion; downstream application and optimization scores for that case also affected | LLM interpretation and directive application are scored separately; a misidentified directive affects both. |
| Directive extracted but not applied (e.g., correct `no_charge_window` interpretation but battery still charges in those hours) | 0 points for directive application on that case | Correct extraction is not enough; the schedule must obey every applicable directive. |
| Energy balance violated or solar over-used | 0 points for constraint correctness on that case | `hourly_plan` must satisfy the energy balance and solar constraints every hour. |
| End-of-day battery neutrality violated | 0 points for constraint correctness on that case | Final `battery_energy_after_kwh` must equal `initial_energy_kwh`. |
| `total_grid_kwh`, `total_cost_bdt`, or `peak_grid_kwh` do not match recalculated values | Reduces API Contract & Schema score | Summary fields must match `hourly_plan` recalculation within tolerance. |
| Service returns 5xx, invalid JSON, or no response for valid requests | Reduces Performance & Reliability score; repeated failures may block qualification eligibility | The service must remain stable and return valid responses throughout the evaluation window. |
| Endpoint unreachable or exceeds 30-second timeout | Reduces Performance & Reliability score; repeated failures may block qualification eligibility | Reliability and latency are scored through the p95 and failure-rate metrics. |
| `hourly_plan` or repeated critical invalidity | Reduces Performance & Reliability score; repeated failures may block qualification eligibility | Structural invalidity (wrong number of hours, missing fields) reduces API contract totals and validity. |
