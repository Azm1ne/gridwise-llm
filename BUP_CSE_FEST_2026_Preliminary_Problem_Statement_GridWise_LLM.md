...
```
minimize: Σ (grid_kwh[h] × tariff_bdt_per_kwh[h])   for h = 0..23
```

Subject to all applicable constraints (Section 09) and operator directives (Section 04).

**Constraint summary by directive:**

| Directive | Downstream constraint |
|---|---|
| `solar_reduction` | `effective_solar_kwh[h] = solar_kwh[h] × factor` for listed hours |
| `minimum_battery_reserve` | `battery_energy_after_kwh[h] >= max(base minimum_energy_kwh, directive minimum_energy_kwh)` for listed hours |
...
| Method | Path | Purpose |
|---|---|---|
| `GET` | `/health` | Readiness check. Must return `{"status": "ok"}` within 60 seconds of service start. |
| `POST` | `/optimize-energy` | Main endpoint. Accepts scenario + operator notes; returns directive interpretation + 24-hour plan. |
...
- Return the correct `directive_type`.
